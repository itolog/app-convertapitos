...
export const makeZodI18nMap: MakeZodI18nMap = (option) => (issue, ctx) => {
  ...
  const path: { path?: string } =
    issue.path.length > 0 && tForm ? { path: tForm(issue.path.join(".")) } : {};
  ...
  switch (issue.code) {
    ...
    case ZodIssueCode.too_small: {
      ...
      message = t(
        ...
        {
          minimum: issue.type === "date" ? minimum : String(minimum), // Convert number to string for display
          ...(issue.type !== "date" ? { count: minimum } : {}), // Only include count for number/string types
          ...path,
        },
      );
      break;
    ...
    case ZodIssueCode.too_big: {
      ...
      message = t(
        ...
        {
          maximum: issue.type === "date" ? maximum : String(maximum), // Convert number to string for display
          ...(issue.type !== "date" ? { count: maximum } : {}), // Only include count for number/string types
          ...path,
        },
      );
      break;
